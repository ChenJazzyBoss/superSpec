...
```
You: "Add batch export"
          │
          ▼
   ┌─────────────┐
   │  superSpec   │  ← Generates structured spec
   │  validate    │  ← Checks completeness
   │  plan        │  ← Creates implementation plan
   └──────┬──────┘
          │
          ▼
   Claude writes code that actually matches your requirements
```
...
brainstorm → generate-spec → validate-spec → write-plan → implement → verify → archive
     │              │              │              │            │          │         │
     │              │              │              │            │          │         │
  Collect        Create        Check          Split        Build      Prove     Record
  requirements   structured    quality        into tasks   with       it works  the change
                 spec                                   TDD
```
