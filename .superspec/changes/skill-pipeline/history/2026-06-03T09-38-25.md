...
#### Scenario: brainstorm 输出传递给 generate-spec
...
#### Scenario: validate-spec 校验失败阻断传递
...
#### Scenario: validate-spec 失败后自动回退到 generate-spec

Given generate-spec 生成的 spec 文件未通过 validate-spec 校验，错误为"需求文本必须包含 SHALL 或 MUST"
When 管道检测到校验失败
Then 系统 SHALL 自动将控制权回退给 generate-spec 技能，附带校验错误信息，generate-spec 技能根据错误信息修正 spec 后重新提交校验

#### Scenario: verify 失败后重试一次

Given verify 技能因测试超时而失败（非逻辑错误）
When 管道检测到验证失败且失败原因为临时性错误
Then 系统 SHALL 自动重试 verify 技能一次，重试前等待 5 秒，如果重试仍然失败则停止并报告最终错误

#### Scenario: 超过最大重试次数

Given 一个技能已经连续失败 3 次（最大重试次数为 3）
When 管道检测到第 3 次失败
Then 系统 SHALL 停止重试，将完整的失败日志（包含每次重试的错误信息和时间戳）写入上下文，并向用户报告该技能执行失败且已达到最大重试次数
