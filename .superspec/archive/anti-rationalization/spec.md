...
每个技能配置文件 MUST 包含一个红线表（Red Flags），列出该技能中 AI 常见的跳步借口与现实情况的对照，用于在执行过程中进行实时检测和拦截。
...
系统 MUST 维护一个可扩展的反跳步模式库，收录常见的 AI 跳步模式和对应的检测规则，支持持续更新和优化。

#### Scenario: 新模式识别与入库

Given 在技能执行过程中发现了一种新的 AI 跳步行为模式
When 维护者将该模式及其检测规则添加到模式库中
Then 系统在后续所有技能执行中自动应用该检测规则

#### Scenario: 直接编码不写 spec
