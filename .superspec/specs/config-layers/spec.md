...
系统 SHALL 对对象类型的配置项采用深度合并（deep merge）策略，对数组和基本类型采用直接覆盖策略。深度合并时 MUST 递归处理嵌套对象，确保子属性的独立覆盖不会丢失同级的其他属性。
...
When 系统加载并校验该配置文件
Then 系统 SHALL 正常完成校验，不产生任何错误信息
