...
基于实际使用反馈优化 superSpec。核心问题：校验只做格式检查（有没有 SHALL、场景数够够），不做逻辑检查；图表功能承诺了但没实现；spec 和代码之间没有同步机制；写 spec 耗时过长因为工具不帮助用户补全场景。这些缺陷导致工具沦为格式检查器，无法真正帮助设计思考。

<!-- DIAGRAM:flowchart -->

```mermaid
flowchart TD
    A["用户写需求描述"] --> B["校验引擎分析需求"]
    B --> C["自动建议缺失的场景类型"]
    C --> D["用户补充/确认场景"]
    D --> E{"需要深度检查?"}
    E -- 是 --> F["逻辑一致性分析"]
    ...
    G --> H
    H --> I{"spec 关联了源码?"}
    I -- 是 --> J["检测代码变更，提醒 spec 是否过时"]
    I -- 否 --> K["跳过同步检查"]
```

## Requirements

### Requirement: 校验引擎 SHALL 自动建议缺失的场景类型
校验引擎 SHALL 分析已有场景的类型覆盖（正常流程、异常处理、边界条件），当某种类型缺失时 MUST 输出建议，告诉用户还需要补充哪些类型的场景。场景数量要求保持不变（每条需求至少 2 个场景），但工具 MUST 帮助用户高效补全。

#### Scenario: 检测缺失的异常处理场景并给出建议
Given 一条需求有 2 个场景，全部是正常流程（When 部分不含错误/失败/超时等关键词）
When 校验引擎检查该需求
Then MUST 报告 WARNING 并输出建议："该需求缺少异常处理场景，建议补充：1) 输入数据校验失败时的处理 2) 操作超时或服务不可用时的处理"

#### Scenario: 检测缺失的边界条件场景并给出建议
Given 一条需求描述了数量限制（如"最多 100 条"），但场景中没有测试边界值
When 校验引擎检查该需求
Then MUST 报告 WARNING 并输出建议："该需求涉及数量限制，建议补充边界条件场景：1) 恰好达到上限 2) 超出上限"

#### Scenario: 场景覆盖完整时不做提示
Given 一条需求有 3 个场景，分别覆盖正常流程、异常处理和边界条件
When 校验引擎检查该需求
Then MUST NOT 报告关于场景类型的 WARNING

#### Scenario: 建议内容 MUST 基于需求描述推断
Given 需求描述说"系统 SHALL 支持 CSV、XLSX、PDF 三种格式"
When 校验引擎生成场景建议
Then 建议 MUST 包含"建议补充 XLSX 和 PDF 格式的测试场景"，而不是通用模板
...
校验规则 SHALL 根据场景的 Given/When/Then 内容自动判断场景类型（正常流程、异常处理、边界条件），并在校验报告中标注每个场景的类型，为需求 1 的建议功能提供基础。
