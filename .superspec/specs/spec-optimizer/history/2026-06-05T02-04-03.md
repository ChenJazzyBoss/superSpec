...
```mermaid
flowchart TB
  subgraph spec["📋 spec-optimizer"]
    direction TB
    subgraph req0["🔑 Requirement: 校验引擎 SHALL 支持逻辑一致性检查"]
      R0S0["📝 Scenario: 检测场景间的逻辑矛盾"]
      R0S1["📝 Scenario: 检测场景未覆盖需求约束"]
      R0S2["📝 Scenario: 未指定 --deep 时跳过逻辑检查"]
      R0S3["📝 Scenario: 逻辑检查结果不阻断校验"]
    end
    subgraph req1["🔑 Requirement: 校验引擎 SHALL 自动生成 Mermaid 图表"]
      R1S0["📝 Scenario: 从需求场景生成流程图"]
      R1S1["📝 Scenario: 从需求场景生成状态图"]
      R1S2["❌ Scenario: 图表生成失败时不阻断校验"]
      R1S3["📝 Scenario: 已有图表标记时更新而非重复插入"]
    end
    subgraph req2["🔑 Requirement: 校验引擎 SHALL 支持 spec-code 关联追踪"]
      R2S0["📝 Scenario: spec 声明关联源码文件"]
      R2S1["📝 Scenario: 源码比 spec 更新时提醒"]
      R2S2["📝 Scenario: 源码文件不存在时提示"]
    end
    subgraph req3["🔑 Requirement: 规则引擎 SHALL 按场景类型识别并分类"]
      R3S0["❌ Scenario: 自动识别异常处理场景"]
      R3S1["⚠️ Scenario: 自动识别边界条件场景"]
      R3S2["✅ Scenario: 默认标记为正常流程"]
      R3S3["❌ Scenario: 缺少异常处理场景时给出提示"]
    end
  end
...
当 spec 中包含 `<!-- DIAGRAM:flowchart -->` 或 `<!-- DIAGRAM:state -->

```mermaid
stateDiagram-v2
  [*] --> 待处理
  待处理 --> 和转换从场景的 : Scenario: 从需求场景生成状态图
  完成 --> [*]
```
...
stateDiagram-v2
  ...
  待处理 --> 和转换从场景的 : Scenario: 从需求场景生成状态图
  完成 --> [*]
```` 等标记时，校验引擎 SHALL 根据 spec 内容自动生成对应的 Mermaid 图表代码并嵌入 spec 文件。
